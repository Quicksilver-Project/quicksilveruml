 ...
 * Description
 * 
 * A RedefinableTemplateSignature
 ...
 * 
 * The Classifier that owns
 * this RedefinableTemplateSignature.
 * 
 *  extendedSignature : RedefinableTemplateSignature [0..*]
 * {subsets RedefinableElement::redefinedElement}
 * (opposite A_extendedSignature_redefinableTemplateSignature
 * ::redefinableTemplateSignature)
 * 
 * The signatures extended by this
 * RedefinableTemplateSignature.
 * 
 ...
 * 
 * body: if extendedSignature->isEmpty()
 * then Set{} else extendedSignature.parameter->asSet()
 * endif
 * 
 *  isConsistentWith(redefiningElement : RedefinableElement)
 * : Boolean {redefines
 * RedefinableElement::isConsistentWith()}
 * 
 * The query isConsistentWith() specifies,
 * for any two RedefinableTemplateSignatures
 * in a context in which redefinition is possible,
 * whether redefinition would be logically consistent.
 * A redefining template signature is
 * always consistent with a redefined
 * template signature,
 * as redefinition only
 * adds new formal parameters.
 * 
 ...
 *  redefines_parents
 * 
 * If any of the parent Classifiers
 * are a template, then the
 * extendedSignature must
 * include the signature of that
 * 
 * Classifier.
 * 
