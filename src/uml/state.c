 ...
 * 
 * A state with isComposite=true is said
 * to be a composite State.
 * A composite State is a State
 * that contains at least one Region.
 * 
 *  /isOrthogonal : Boolean [1..1]
 * 
 * A State with isOrthogonal=true 
 * is said to be an orthogonal composite State.
 * An orthogonal composite State
 * contains two or more Regions.
 * 
 *  /isSimple : Boolean [1..1]
 * 
 * A State with isSimple=true
 * is said to be a simple State.
 * A simple State does not have any Regions and it does
 * not refer to any submachine StateMachine.
 * 
 *  /isSubmachineState : Boolean [1..1]
 * 
 * A State with isSubmachineState=true
 * is said to be a submachine State.
 * Such a State refers to another
 ...
 * 
 * The entry and exit connection points used
 * in conjunction with this (submachine) State,
 * i.e., as targets and sources, respectively,
 * in the Region with the submachine State.
 * A connection point reference references the
 * corresponding definition of a connection point
 * Pseudostate in the StateMachine
 ...
 * These can only be entry or exit Pseudostates,
 ...
 * 
 * A list of Triggers that are candidates
 * to be retained by the StateMachine
 * if they trigger no Transitions out of the
 * State (not consumed). A deferred Trigger
 * is retained until the StateMachine
 * reaches a State configuration where
 * it is no longer deferred.
 * 
 *  ♦ doActivity : Behavior [0..1]
 * {subsets Element::ownedElement}
 * (opposite A_doActivity_state::state)
 * 
 * An optional Behavior that is executed
 * while being in the State.
 * The execution starts when this State is entered,
 * and ceases either by itself when done,
 * or when the State is exited,
 * whichever comes first.
 * 
 *  ♦ entry : Behavior [0..1]
 * {subsets Element::ownedElement}
 * (opposite A_entry_state::state)
 * 
 * An optional Behavior that is executed
 * whenever this State is entered
 * regardless of the Transition taken to reach
 * the State. If defined,
 * entry Behaviors are always executed 
 * to completion prior to any 
 ...
 *  ♦ exit : Behavior [0..1]
 * {subsets Element::ownedElement}
 * (opposite A_exit_state::state)
 * 
 * An optional Behavior that is executed 
 * whenever this State is exited regardless 
 * of which Transition was taken
 * out of the State. If defined, 
 * exit Behaviors are always executed to 
 * completion only after all internal and
 ...
 * 
 * The State of which this
 * State is a redefinition.
 * 
 ...
 * 
 * Specifies conditions that are always true
 * when this State is the current State.
 * In ProtocolStateMachines state
 * invariants are additional conditions to 
 * the preconditions of the outgoing Transitions, 
 * and to the postcondition of
 ...
 * in place of the (submachine) State.
 * 
 * Operations
 * 
 *  containingStateMachine() : StateMachine 
 * {redefines Vertex::containingStateMachine()}
 * 
 * The query containingStateMachine() returns the 
 * StateMachine that contains 
 ...
 * 
 * UMLR-685: StateMachine Vertex needs to be made
 * a kind of RedefinableElement instead of State
 * 
 *  isConsistentWith(redefiningElement : 
 * RedefinableElement) : Boolean 
 * {redefines
 * RedefinableElement::isConsistentWith()}
 * 
 * The query isConsistentWith() specifies that a 
 * redefining State is consistent with a 
 * redefined State provided that
 * the redefining State is an extension of the 
 * redefined State.
 * A simple State can be redefined (extended) to
 * become a composite State 
 * (by adding one or more Regions) 
 * and a composite State can be redefined (extended)
 * by adding Regions and by adding Vertices, 
 * States, and Transitions 
 * to inherited Regions. 
 * All States may add or
 * replace entry, exit, and 'doActivity' 
 * Behaviors. The query isConsistentWith 
 * specifies that a non-final State can
 * only be redefined by a non-final State 
 * (this is overridden by FinalState to allow 
 * a FinalState to be redefined by a FinalState)
 * and, if the redefined State is a submachine State,
 * then the redefining State must be a submachine
 * state whose submachine is a redefinition of the
 * submachine of the redefined State.
 * Note that consistency
 * requirements for the redefinition of Regions
 * and connectionPoint Pseudostates within a composite State and
 * connection ConnectionPoints of a submachine State
 * are specified by the isConsistentWith and
 * isRedefinitionContextValid operations for 
 * Region and Vertex (and its subclasses, Pseudostate and
 ...
 * 
 * UMLR-685: StateMachine Vertex needs to be made a kind of 
 * RedefinableElement instead of State
 * 
 *  redefinitionContext() : Classifier
 * 
 * The redefinition context of a State is
 * the nearest containing StateMachine.
 * 
 ...
 * 
 * The connection point references used as destinations/sources 
 * of Transitions associated with a submachine State
 * must be defined as entry/exit points in the 
