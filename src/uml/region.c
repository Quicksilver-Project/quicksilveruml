 ...
 * Description
 * 
 * A Region is a top-level part
 * of a StateMachine
 * or a composite State,
 * that serves as
 * a container for
 * the Vertices and
 * Transitions of
 ...
 * (opposite A_extendedRegion_region::region)
 * 
 * The region of which this region is an extension.
 * 
 *  /redefinitionContext : Classifier [1..1]
 * {redefines RedefinableElement::redefinitionContext}
 * (opposite A_redefinitionContext_region::region)
 * 
 * References the Classifier
 * in which context
 ...
 * If a Region is owned by a State,
 * then it cannot also be owned by a
 * StateMachine.
 * 
 ...
 * If a Region is owned by a StateMachine,
 * then it cannot also be owned
 * by a State.
 * 
 *  ♦ subvertex : Vertex [0..*]
 ...
 * 
 * The operation belongsToPSM() 
 * checks if the Region 
 * belongs to a ProtocolStateMachine.
 * 
 ...
 * 
 * The query isConsistentWith()
 * specifies that a redefining 
 * Region is consistent with a 
 * redefined Region provided
 * that the redefining Region
 * is an extension of the
 * Redefined region, i.e.,
 * its Vertices and Transitions
 ...
 * redefined Region or,
 * 
 * (2) they consistently redefine a 
 * State or Transition of the 
 * redefined region, or 
 * 
 * (3) they add new States or Transitions.
 * The query isConsistentWith specifies
 * that a Region can be
 * redefined by any Region
 * for which the redefinition context is valid
 * (see the isRedefinitionContextValid operation). 
 * Note that consistency requirements for the
 * redefinition of Vertices and Transitions
 * within a redefining Region are specified
 * by the isConsistentWith and
 * isRedefinitionContextValid operations 
 * for Vertex (and its subclasses) and Transition.
 * 
 ...
 * {redefines RedefinableElement::isRedefinitionContextValid()}
 * 
 * The query isRedefinitionContextValid()
 * specifies whether the redefinition
 * contexts of a Region are properly
 * related to the redefinition contexts of the
 * specified Region to allow this
 * element to redefine the other.
 * The containing StateMachine or State 
 * of a redefining Region must Redefine
 * the containing StateMachine or State
