 ...
 * Description
 * 
 * An UnmarshallAction is an Action
 * that retrieves the values of the
 * StructuralFeatures of an
 * object and places them on
 * OutputPins.
 * 
 * Diagrams
 * 
 ...
 * (opposite A_object_unmarshallAction::unmarshallAction)
 * 
 * The InputPin that gives the
 * object to be unmarshalled.
 * 
 *  ♦ result : OutputPin [1..*]
 * {ordered, subsets Action::output}
 * (opposite
 * A_result_unmarshallAction
 * ::unmarshallAction)
 * 
 * The OutputPins on which are
 * placed the values of the
 ...
 *  number_of_result
 * 
 * The number of result outputPins must
 * be the same as the number of 
 * attributes of the unmarshallType.
 * 
 * inv: unmarshallType.allAttributes()->size() = result->size()
 * 
 *  type_ordering_and_multiplicity
 * 
 * The type, ordering and multiplicity of 
 * each attribute of the unmarshallType 
 * must be compatible with the type,
 * ordering and multiplicity of the 
 * corresponding result OutputPin.
 * 
 ...
 *  object_type
 * 
 * The type of the object InputPin
 * conform to the unmarshallType.
 * 
